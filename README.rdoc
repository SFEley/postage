...
There are several Rake tasks to test and use Postage within a Rails application.

To have a look at the current configuration based on the Rails environment:

  rake postage:config

To test that the configuration file is correctly installed and the API key is valid:

  rake postage:test

To see what transactions have failed to be sent to Postage and are saved in a local
store:

  rake postage:queue
  
To attempt delivery of the locally queued items:
